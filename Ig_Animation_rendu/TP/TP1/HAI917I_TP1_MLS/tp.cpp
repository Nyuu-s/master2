...
void HPSS(  Vec3 inputPoint,
             ...
             unsigned int knn = 20 ) 
    {
        ...
        for(int i = 0; i<k; i++)
        {
            projections[i] = projectPointPlane(inputPoint, positions[id_nearest_neighbors[i]], normals[id_nearest_neighbors[i]] );
            centroide += projections[i];
            avgNormal += normals[id_nearest_neighbors[i]];
        }

        centroide /= k;
        avgNormal /= k;
        outputPoint = projectPointPlane(inputPoint, centroide, avgNormal );
        outputNormal = avgNormal;
